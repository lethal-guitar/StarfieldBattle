...
class Sprite
{
	...
	public:
		...
		void SetXY(int PosX, int PosY) { x = float(PosX); y = float(PosY); }
		void Move(float MvX, float MvY) { if ((x+MvX > 0) && (x+FrameWidth+MvX  < SCR_WIDTH))  x += MvX; 
									  if ((y+MvY > 0) && (y+FrameHeight+MvY < SCR_HEIGHT)) y += MvY; } 
		void Draw(LPDIRECTDRAWSURFACE7);
		void Reset(void) { if (Continue == FALSE) { Continue = TRUE; Done = FALSE; } }
		POINT GetXY(BOOL AddHalfSizeX = TRUE)   { POINT p = { int(x), int(y) }; if (AddHalfSizeX) p.x = int(x)+FrameWidth/2; return p; }
		POINT GetSize(void) { POINT p = { FrameWidth, FrameHeight }; return p; }
		void Bounce(void);
		...
		void SetNewBBox(int x1, int y1, int x2, int y2) { SetRect(&BBox, x1, y1, x2, y2); }
...
class ASprite_NoList : public Sprite
{
	...
	public:
		...
		{ 
			this->Type = Type; ShotSnd = Sound; ShotDelay = Delay; 
		}

		void IncShotDelay(void) { if (++Time >= FrameRate / 60) { Time = 0; CurrentShotDelay++; } /*else CanFire = FALSE;*/ }
