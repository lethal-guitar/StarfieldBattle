...
int DirectInputManager::GetMouseInput(void)
{
  ...
  if (FAILED(lpDIDevice->GetDeviceData(
        sizeof(data), &data, (DWORD*)&NumElements, 0)))
  {
    lpDIDevice->Acquire();
    return 0;
  }
...
BOOL CALLBACK DirectInputManager::EnumJoystickCallback(
  const DIDEVICEINSTANCE* lpDInstance,
  LPVOID lpContext)
{
  memcpy(
    lpContext, &lpDInstance->guidInstance, sizeof(lpDInstance->guidInstance));
