...
Sprite::Sprite(void)
{
	...
	Looped      = Done = TRUE;
	Continue    = FALSE;
	FrameWidth  = FrameHeight = FrameDelay = Delay = CurrentFrameX = CurrentFrameY = FramesX = FramesY = StepX = StepY = 0;
	x           = y = 0.0f;
	BBox.left   = BBox.right  = BBox.top   = BBox.bottom           = 0;
}

void Sprite::Create(LPDIRECTDRAWSURFACE7 lpDDS, int FrmWidth, int FrmHeight, int FrmDelay, BOOL IsLooped, int StX, int StY)
{
	StepX = StX; StepY = StY;
	...
	FrameWidth = FrmWidth; FrameHeight = FrmHeight;
...
void Sprite::Draw(LPDIRECTDRAWSURFACE7 lpDDS)
{
	...
	if(FAILED(lpDDS->BltFast(DWORD(x), DWORD(y), lpDDSSprite, &r, DDBLTFAST_SRCCOLORKEY | DDBLTFAST_WAIT)))
	{
		if (x < 0)
		{
			x = 0;
		...
		lpDDS->BltFast(DWORD(x), DWORD(y), lpDDSSprite, &r, DDBLTFAST_SRCCOLORKEY | DDBLTFAST_WAIT);
	}
