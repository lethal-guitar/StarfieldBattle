...
static BOOL Running = TRUE, InGame = FALSE, QuitQuery = FALSE, PlayerDieSequence = FALSE;
...
LRESULT CALLBACK WindowProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYDOWN:
		...
		{
		case VK_ESCAPE:
			if (Running && InGame) QuitQuery = TRUE;
			//DestroyWindow(hWnd);
			break;

		case 0x50: // VK_P
		case VK_PAUSE:
			if (!InGame || PlayerDieSequence) break;
			
			Running = !Running; 
			
			RECT r = { 0, 0, 500, 200 };
			DDM.GetBuffer()->BltFast(150, 200, lpDDSLogo, &r, DDBLTFAST_SRCCOLORKEY | DDBLTFAST_WAIT);
...
void ShowText(UINT Alignment, COLORREF Color, int x, int y, LPCTSTR Text, LPDIRECTDRAWSURFACE7 lpDDS)
{
	HDC hDC;
...
BOOL DoMainInits(void)
{
	...
	if (!DMM.AddSndFX("PlShoot.wav"))   return FALSE;
	if (!DMM.AddSndFX("Enemy1.wav"))    return FALSE;
	if (!DMM.AddSndFX("Enemy2.wav"))    return FALSE;
	if (!DMM.AddSndFX("Enemy3.wav"))    return FALSE; 
	if (!DMM.AddSndFX("Explosion.wav")) return FALSE;
	//if (!DMM.AddSndFX("Brrzz.wav"))     return FALSE;
	if (!DMM.AddBGMusic("Music.mid"))   return FALSE;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	LPDIRECTDRAWSURFACE7 lpDDSPlayer  = DDM.CreateSurfaceFromBitmap("Sprites\\Player.bmp",  150, 120);
	LPDIRECTDRAWSURFACE7 lpDDSPlayer2 = DDM.CreateSurfaceFromBitmap("Sprites\\Player2.bmp", 150, 120);
	LPDIRECTDRAWSURFACE7 lpDDSPlShot  = DDM.CreateSurfaceFromBitmap("Sprites\\PlShoot.bmp",  60,  20);
	LPDIRECTDRAWSURFACE7 lpDDSPlShot2 = DDM.CreateSurfaceFromBitmap("Sprites\\PlShoot2.bmp", 60,  20);
	                     lpDDSLogo    = DDM.CreateSurfaceFromBitmap("Logos.bmp", 500, 665);
	LPDIRECTDRAWSURFACE7 lpDDSMenu    = DDM.CreateSurfaceFromBitmap("Menu.bmp",  460, 800);
	//LPDIRECTDRAWSURFACE7 lpDDSPlMuzzle = DDM.CreateSurfaceFromBitmap("Sprites\\Player_Muzzle.bmp", 56, 26);
	...
	lpDDSBackBuffer->BltFast(150, 200, lpDDSLogo, &r, DDBLTFAST_SRCCOLORKEY | DDBLTFAST_WAIT);
	DDM.Flip();
	DDM.CopyPrimaryToBackBuffer();
	...
	Player.SetShotData(lpDDSPlShot, 12, 20, MSECS(16), TRUE, 3, -600, "PlShoot.wav", 25);
	//Player.SetMuzzleFlash(lpDDSPlMuzzle);
	Player.SetNewBBox(8, 28, 65, 84);
		
	Player2.Create(lpDDSPlayer2, 75, 120,  MSECS(80), TRUE);
	Player2.SetShotData(lpDDSPlShot2, 12, 20, MSECS(16), TRUE, 3, -600, "PlShoot.wav", 25);
	...
	{
		short Nr = fread((void*)Entrys, sizeof(HiScoreEntry), MAX_HISCORE_ENTRYS, fScoreList);
...
/* ----------> */	if (Running && !PlayerDieSequence)
					{
						...
						if (!MultiPlayer)
						{
							for (; i < PlHealth-1; i++)
							{
								lpDDSBackBuffer->BltFast(PlHlthPos.x + 65*i, PlHlthPos.y, lpDDSLogo, &PlFrmSize, DDBLTFAST_SRCCOLORKEY | DDBLTFAST_WAIT);
							}
						...
						if (!MultiPlayer)
						{
							...
							if (CollisionValue & PLAYER_IS_HIT)
							{
								...
								{	
									...
									if (PlHealth <= 0)
									{	
										FXM.CreateExplosionAt(Player.GetXY(FALSE).x, Player.GetXY().y);
						...
						else // MultiPlayer == TRUE
						{
							...
							sprintf(Str, "%2d:%02d", MultiPlayerTime / 60, MultiPlayerTime % 60);
							
							ShowText(TA_CENTER, RGB(255, 255, 255), 380, 0, Str, lpDDSBackBuffer);
							
							int Pl2ScoreAmount;

							int Pl2CollisionValue = Enemys.TestCollisions(&Player2, &Pl2ScoreAmount);
						
							if (CollisionValue & PLAYER_IS_HIT)
								if (PlPoints > -20) PlPoints -= 10;

							if (CollisionValue & PLAYER_HAS_SCORED)
								PlPoints += ScoreAmount * 5;

							if (Pl2CollisionValue & PLAYER_IS_HIT)
								if (Pl2Pts > -20) Pl2Pts -= 10;
...
/* ----------> */	else if (PlayerDieSequence)
					{
						...
						else // GameOver == TRUE
						{
							...
							lpDDSBackBuffer->BltFast(155, 50, lpDDSLogo, &r, DDBLTFAST_SRCCOLORKEY | DDBLTFAST_WAIT);
							...
							if (!MultiPlayer)
							{
								...
								ShowText(TA_CENTER, RGB(255, 255, 255), 380, 250, str, lpDDSBackBuffer);
								...
								if (NewScoreEntry)
								{
									...
									if (RGBFadeIn >= 250)
									{
										...
										ShowText(TA_CENTER, RGB(255, 255, 255), 380, 280, str, lpDDSBackBuffer);
									}
								
									sprintf(str, "You made a new High Score!");
									ShowText(TA_CENTER, RGB(0, RGBFadeIn, 0), 380, 265, str, lpDDSBackBuffer);
									...
									else if (ScanCode == DIK_RETURN && PlayerName[0])
									{
										...
										if (i >= MAX_HISCORE_ENTRYS)
										{
											Error("Achtung: Eintrag in HighScore-Liste war \
												möglich, obwohl Punktzahl nicht hoch genug war. \
												Dies sollt überhaupt nicht möglich sein, deshalb \
									...
									if (RGBFadeIn > 250 && Typed > 0 && strlen(PlayerName) < 49  && (isalpha(Typed) || isspace(Typed) || isalnum(Typed) || ispunct(Typed)) && ScanCode != DIK_RETURN)
									{
									...
									ShowText(TA_CENTER, RGB(255, 0, 0), 380, 300, PlayerName, lpDDSBackBuffer);
								}
								else // NewScoreEntry == FALSE
								{
									ShowText(TA_CENTER, RGB(RGBFadeIn, 0, 0), 380, 265, "Sorry, you didn't make a High Score!", lpDDSBackBuffer);
								}
							}
							else // MultiPlayer == TRUE
							{	
								...
								if (PlPoints > Pl2Pts)
									sprintf(Str, "Player 1 has won the match %d : %d !", PlPoints, Pl2Pts);
								else
									sprintf(Str, "Player 2 has won the match %d : %d !", Pl2Pts, PlPoints); 
									
								ShowText(TA_CENTER, RGB(RGBFadeIn, 0, 0), 380, 265, Str, lpDDSBackBuffer);
							}
				...
				else // Not in game
				{
					...
					{
						ShowText(TA_CENTER, RGB(255, 255, 255), 220, 180, "Name",  lpDDSBackBuffer);
						ShowText(TA_CENTER, RGB(255, 255, 255), 580, 180, "Score", lpDDSBackBuffer);
						...
						for (int i = 0; i < MAX_HISCORE_ENTRYS; i++)
						{
							...
							ShowText(TA_LEFT,  Color, 200, 200 + i*20, Entrys[i].Name,  lpDDSBackBuffer);
							ShowText(TA_RIGHT, Color, 600, 200 + i*20, strScore,        lpDDSBackBuffer);
						}
						...
						ShowText(TA_CENTER, RGB(RGBFade, RGBFade, RGBFade), 410, 220 + MAX_HISCORE_ENTRYS*20, "Press any key to return to main menu", lpDDSBackBuffer);
